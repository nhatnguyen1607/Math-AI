...
const FacultyStudentExamResultPage = () => {
  ...
  useEffect(() => {
    const loadResults = async () => {
      ...
      try {
        ...
        try {
          const leaderboard =
            await facultyService.getExamStudentResults(examId);
          ...
          if (studentData) {
            ...
            setStudent({
              uid: userId,
              name:
                result.data?.studentName || `Student ${userId.substring(0, 8)}`,
              rank: "?",
  ...
  const isCompetencyCompletionValid = useCallback(() => {
    try {
      ...
      const khoiDongCompleted =
        khoiDongAnswers &&
        ((Array.isArray(khoiDongAnswers) && khoiDongAnswers.length > 0) ||
          (typeof khoiDongAnswers === "object" &&
            Object.keys(khoiDongAnswers).length > 0));
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-pink-100">
                Xếp hạng: <strong>#{student?.rank || "-"}</strong>
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="p-6 lg:p-8 bg-indigo-50 space-y-6 lg:space-y-8 rounded-2xl">
                ...
                {exam?.exercises && exam.exercises.length > 0 ? (
                  exam.exercises.map((exercise, exerciseIdx) => (
                    <div
                      key={exerciseIdx}
                      className="bg-white rounded-3xl p-5 lg:p-6 shadow-soft-md border border-indigo-100"
                    >
                      {/* Exercise Header */}
                        ...
                        {exercise.questions && exercise.questions.length > 0 ? (
                          exercise.questions.map((question, questionIdx) => {
                            ...
                            for (let i = 0; i < exerciseIdx; i++) {
                              globalQuestionIndex +=
                                exam.exercises[i].questions?.length || 0;
                            }
                            ...
                            if (
                              Array.isArray(
                                studentResult.data?.parts?.khoiDong?.answers,
                              )
                            ) {
                              // Array format - find by questionIndex or use index directly
                              answerData =
                                studentResult.data.parts.khoiDong.answers.find(
                                  (a) =>
                                    a.questionIndex === globalQuestionIndex,
                                ) ||
                                studentResult.data.parts.khoiDong.answers[
                                  globalQuestionIndex
                                ];
                            ...
                            const isExpanded =
                              exerciseIdx === 0 ||
                              expandedQuestions[globalQuestionIndex];
                                ...
                                {isExpanded && (
                                  <div className="p-8">
                                      ...
                                      {(question.options || []).map(
                                        (option, oIdx) => {
                                          ...
                                          const isCorrectAnswer =
                                            answerData.correctAnswerIndex ===
                                            oIdx;

                                          // Hiển thị "✓ Bạn chọn" (green) khi học sinh chọn và câu đúng
                                          const showAsCorrect =
                                            isSelected && answerData.isCorrect;
                                    ...
                                    {(() => {
                                      ...
                                      return (
                                        <div
                                          className={`p-6 border-l-4 rounded-2xl bg-indigo-100 border-indigo-600`}
                                        >
            ...
            {loadingPractice ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-lg">
                  Học sinh chưa hoàn thành luyện tập
                </p>
              </div>
            ) : (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {["TC1", "TC2", "TC3", "TC4"].map((tc) => {
                                ...
                                return (
                                  <div
                                    key={tc}
                                    className={`p-5 rounded-lg border-l-4 border-b border-r ${levelColor}`}
                                  >
                                    ...
                                    <p
                                      className={`text-sm leading-relaxed whitespace-pre-wrap ${textColor}`}
                                    >
                                      {tcData?.nhanXet || ""}
                                    </p>
                                  </div>
                                  ...
                                  <div className="text-right">
                                    ...
                                    <p
                                      className={`text-lg font-bold ${
                                        evaluation.mucDoChinh === "Tốt"
                                          ? "text-green-600"
                                          : evaluation.mucDoChinh === "Đạt"
                                            ? "text-blue-600"
                                            : "text-orange-600"
                                      }`}
                                    >
                                      {evaluation.mucDoChinh || "-"}
                                    </p>
                                  </div>
                                </div>
                              </div>
          ...
          <div className="bg-white rounded-3xl shadow-soft-lg p-6 lg:p-8 border-t-4 border-yellow-300">
            ...
            {loadingPractice ? (
              ...
              <div className="text-center py-12">
                <div className="text-6xl mb-4">⚡</div>
                <p className="text-gray-500 text-lg">
                  Học sinh chưa hoàn thành phần vận dụng
                </p>
              </div>
            ) : (
              <>
                ...
                <div className="bg-gradient-to-br from-yellow-50 via-orange-50 to-red-50 rounded-3xl p-6 lg:p-8 border-3 border-yellow-200">
                  ...
                  <div className="mb-6 pb-4 border-b-3 border-yellow-300">
                    ...
                    <p className="text-gray-700 text-sm">
                      ...
                      {practiceData.vanDung?.deBai || "Không có dữ liệu"}
                    </p>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl p-6 mb-6 max-h-96 overflow-y-auto border border-yellow-200">
                    ...
                    <div className="space-y-4">
                      {practiceData.vanDung?.chatHistory &&
                      practiceData.vanDung.chatHistory.length > 0 ? (
                  ...
                  practiceData.vanDung?.evaluation ? (
                    <div className="space-y-6">
                      ...
                      <div className="bg-gradient-to-br from-yellow-50 to-orange-50 p-6 rounded-lg border-l-4 border-orange-500 space-y-4">
                        ...
                        <div
                          className={`pt-3 ${practiceData.vanDung.evaluation?.tongNhanXet ? "border-t border-orange-200" : ""}`}
                        >
                          <div className="flex justify-between items-center">
                            <div>
                              <p className="text-sm text-gray-600 font-medium">
                            ...
                            <div className="text-right">
                              ...
                              >
                                {practiceData.vanDung.evaluation?.mucDoChinh ||
                                  "-"}
                              </p>
                            </div>
                          </div>
                    ...
                    <div className="bg-yellow-50 p-6 rounded-lg border-l-4 border-yellow-500 text-center">
                      <p className="text-gray-600">
                        Phần vận dụng chưa được đánh giá
                      </p>
          ...
          <div className="bg-white rounded-3xl shadow-soft-lg p-6 lg:p-8 border-t-4 border-purple-300">
            ...
            {!isCompetencyCompletionValid() ? (
              ...
              (() => {
                ...
                const khoiDongTotal =
                  khoiDongEval.totalCompetencyScore ||
                  (khoiDongEval.TC1?.score || 0) +
                    (khoiDongEval.TC2?.score || 0) +
                    ...
                    <div className="space-y-6">
                      ...
                      {["TC1", "TC2", "TC3", "TC4"].map((tc) => {
                        ...
                        return (
                          <div
                            key={tc}
                            className="bg-white rounded-3xl p-6 lg:p-8 shadow-soft hover:shadow-soft-lg transition-shadow border border-gray-200"
                          >
                            {/* TC Header */}
                      ...
                      {loadingAiAssessment ? (
                        <div className="text-center py-8">
                          ...
                          <p className="text-indigo-100">
                            Đang phân tích phát triển của học sinh...
                          </p>
                        </div>
                      ) : (
                        <div className="space-y-4">
                          {aiAssessment && (
                            <div className="flex items-start gap-3">
                              ...
                              <p className="text-lg leading-relaxed whitespace-pre-wrap text-indigo-50">
                                {aiAssessment}
                              </p>
                            </div>
                          )}
