...
const StudentWorksheetPage = ({ user, onSignOut }) => {
  ...
  const [showBai3BaiLamFractionDialog, setShowBai3BaiLamFractionDialog] =
    useState(false);
  const [showBai3GiaiThichFractionDialog, setShowBai3GiaiThichFractionDialog] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        // Merge saved Bài 2 arrangements with any missing cách_X keys
        const mergedArr = data.bai2Arrangements || {};
        if (worksheetData.bai_2?.so_cach_giai) {
          for (
            let i = 1;
            i <= parseInt(worksheetData.bai_2.so_cach_giai);
            i++
          ) {
            if (!(`cach_${i}` in mergedArr)) {
              mergedArr[`cach_${i}`] = [];
            }
          }
        }
        setBai2Arrangements(mergedArr);

        setBai3BaiLam(data.bai3BaiLam || "");
        setBai3GiaiThich(data.bai3GiaiThich || "");

        // Merge saved Bài 4 answers with any missing questions
        const mergedAnswers = data.bai4Answers || {};
        if (worksheetData.bai_4?.questions) {
          worksheetData.bai_4.questions.forEach((q) => {
            if (!(q.id in mergedAnswers)) {
              if (q.type === "so_cach_giai") {
                const arrangementsArray = [];
                for (let i = 1; i <= parseInt(q.content); i++) {
                  arrangementsArray.push([]);
                }
                mergedAnswers[q.id] = arrangementsArray;
              } else if (q.type === "cau_hoi_nho") {
                mergedAnswers[q.id] = (q.subQuestions || []).map(() => "");
              } else {
                mergedAnswers[q.id] = "";
              }
            }
  ...
  const insertBai3BaiLamFraction = () => {
    if (bai3BaiLamFractionNum && bai3BaiLamFractionDen) {
      setBai3BaiLam(
        (prev) =>
          prev + `(${bai3BaiLamFractionNum})/(${bai3BaiLamFractionDen})`,
      );
  ...
  const insertBai3GiaiThichFraction = () => {
    if (bai3GiaiThichFractionNum && bai3GiaiThichFractionDen) {
      setBai3GiaiThich(
        (prev) =>
          prev + `(${bai3GiaiThichFractionNum})/(${bai3GiaiThichFractionDen})`,
      );
    ...
    if (bai4FractionNum && bai4FractionDen && currentBai4EditField) {
      ...
      setBai4Answers((prev) => {
        ...
        if (fieldType === "subquestion") {
          ...
          newAnswers[questionId][fieldIndex] =
            (newAnswers[questionId][fieldIndex] || "") + fraction;
  ...
  // Handle Shift+Enter in textarea (allow line break instead of submit)
  const handleTextareaKeyDown = (e) => {
    if (e.key === "Enter" && e.shiftKey) {
      e.preventDefault();
      // Allow default Enter behavior to add newline
      ...
      // Find which state to update based on textarea
      if (textarea.name && textarea.name.startsWith("bai4_")) {
        // eslint-disable-next-line no-unused-vars
        const [_, questionId, fieldType, fieldIndex] = textarea.name.split("_");
        const newAnswers = { ...bai4Answers };
        if (fieldType === "subquestion" || fieldType === "solution") {
          if (!newAnswers[questionId]) newAnswers[questionId] = [];
          newAnswers[questionId][parseInt(fieldIndex)] = newValue;
        } else {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-green-100 via-emerald-100 to-teal-100 px-4">
        ...
        <div className="text-center py-20">
          ...
          <h1 className="mb-4 text-3xl font-bold text-green-700 sm:text-5xl">
            Tuyệt vời!
          </h1>
          <p className="mb-6 text-lg font-semibold text-green-600 sm:mb-8 sm:text-2xl">
            Bạn đã hoàn thành phiếu bài tập này rồi! 🎉
          </p>
          <p className="mb-6 text-base text-gray-700 sm:mb-8 sm:text-lg">
            Mỗi phiếu chỉ làm được một lần thôi.
          </p>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-[7.2rem] z-20 rounded-3xl border-4 border-blue-300 bg-gradient-to-br from-blue-50 to-cyan-50 p-5 shadow-lg sm:top-[8.2rem] sm:p-6">
              ...
              <div className="bg-blue-100 p-5 rounded-2xl border-2 border-blue-300">
                <p className="text-justify text-sm font-semibold leading-relaxed text-gray-800 sm:text-base">
                  <FractionRenderer text={worksheetData.context} />
                </p>
              </div>
            ...
            <div className="space-y-8">
                ...
                <div className="bg-gradient-to-br from-blue-50 to-cyan-100 rounded-3xl shadow-lg p-8 mb-8 border-4 border-blue-300 relative">
                  ...
                  <div className="bg-yellow-100 p-4 rounded-2xl mb-6 border-2 border-yellow-300">
                    <p className="text-sm text-yellow-800 font-bold">
                      ...
                      <strong>
                        Có {worksheetData.bai_2.so_cach_giai} cách giải
                      </strong>
                    </p>
                    <p className="text-sm text-yellow-700 mt-2 font-semibold">
                      {" "}
                      Nhấn [ ➕ Thêm bước ] để chọn bước. 👆
                    </p>
                  </div>
                    ...
                    {Object.keys(bai2Arrangements)
                      ...
                      .map((cach) => (
                        ...
                        >
                          ...
                          <div className="min-h-24 p-4 bg-white rounded-2xl border-3 border-dashed border-blue-400">
                            {bai2Arrangements[cach] &&
                            bai2Arrangements[cach].length > 0 ? (
                              <div className="flex flex-wrap gap-3">
                                {bai2Arrangements[cach].map(
                                  (questionId, index) => {
                                    const question =
                                      worksheetData.bai_2.questions.find(
                                        (q) => q.id === questionId,
                                      );
                                    return (
                                      <button
                                      ...
                                      >
                                        <span className="bg-white text-green-600 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold">
                                          {index + 1}
                                        </span>
                                        <span>
                                          <FractionRenderer
                              ...
                              <p className="text-gray-400 text-lg font-semibold">
                                👇 Chưa có bước nào. Nhấn nút dưới để thêm.
                              </p>
                            )}
                          </div>
                          ...
                          <button
                            ...
                            className="mt-3 w-full rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 text-white py-3 px-4 text-sm font-bold shadow-md active:scale-[0.99]"
                          >
                            [ ➕ Thêm bước ]
                          </button>
                        </div>
                  ...
                  {/* Bài 2 Bottom Sheet / Dialog */}
                  {isBai2BottomSheetOpen && (
                    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4 md:p-0">
                      <button
                        type="button"
                      ...
                      />
                      {/* Mobile: Bottom Sheet */}
                      <div className="md:hidden absolute bottom-0 left-0 right-0 max-h-[70vh] overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl">
                        ...
                        <div className="mb-4 flex items-center justify-between">
                          ...
                          <button
                            type="button"
                            onClick={handleCloseBai2BottomSheet}
                            className="rounded-lg px-3 py-1.5 text-sm font-semibold text-gray-600 hover:bg-gray-100 transition"
                          >
                            ✕
                          </button>
                        </div>
                        <p className="mb-4 text-sm font-semibold text-blue-700 px-1">
                          {activeBai2Cach
                            ? `Thêm bước vào ${activeBai2Cach.replace("cach_", "Cách ")}`
                            : "Chọn cách giải trước khi thêm bước"}
                        </p>
                        <div className="space-y-2">
                          {(worksheetData.bai_2.questions || []).map((q) => (
                            <button
                              ...
                              className="w-full rounded-xl px-4 py-3 text-left text-sm font-bold transition bg-blue-50 text-blue-800 border-2 border-blue-200 hover:bg-blue-100 hover:border-blue-400 active:scale-95"
                            >
                      ...
                      <div className="hidden md:block relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
                        <div className="flex items-center justify-between mb-5">
                          ...
                          <button
                            type="button"
                            onClick={handleCloseBai2BottomSheet}
                            className="rounded-lg px-3 py-2 text-xl font-semibold text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition"
                          >
                            ✕
                          </button>
                        </div>
                        <p className="mb-6 text-base font-semibold text-blue-700 p-3 bg-blue-50 rounded-xl border-2 border-blue-200">
                          {activeBai2Cach
                            ? `Thêm bước vào ${activeBai2Cach.replace("cach_", "🌟 Cách ")}`
                            : "Chọn cách giải trước khi thêm bước"}
                        </p>
                        <div className="grid grid-cols-1 gap-3">
                          {(worksheetData.bai_2.questions || []).map((q) => (
                            <button
                              ...
                              type="button"
                              onClick={() => handleTapAddBai2Step(q.id)}
                              className="w-full rounded-xl px-5 py-4 text-left text-base font-bold transition bg-gradient-to-r from-blue-50 to-blue-100 text-blue-800 border-2 border-blue-300 hover:from-blue-100 hover:to-blue-200 hover:border-blue-500 active:scale-95 shadow-sm hover:shadow-md"
                            >
                              <FractionRenderer text={q.text} />
                            </button>
                          ))}
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
                      <div className="w-full max-w-sm rounded-2xl bg-white p-5 dshadow-xl">
                        ...
                        <p className="mt-2 text-sm text-gray-600">
                          Bạn có muốn xóa bước này không?
                        </p>
              ...
              {worksheetData.bai_3 && (
                <div className="bg-gradient-to-br from-green-50 to-emerald-100 rounded-3xl shadow-lg p-8 mb-8 border-4 border-green-300 relative">
                    ...
                    <div className="p-4 bg-white rounded-2xl border-3 border-green-300 shadow-md">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setShowBai3GiaiThichFractionDialog(true)
                          }
                          disabled={isAlreadySubmitted}
                          className="text-sm font-bold text-white bg-gradient-to-r from-green-500 to-green-600 px-3 py-1 rounded-lg hover:from-green-600 hover:to-green-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                  ...
                  <div className="mt-4 flex items-center gap-2 text-sm text-green-700 font-semibold p-3 bg-green-100 rounded-xl">
                    <span className="text-xl">⭐</span>
                    <span>
                      Hãy viết rõ ràng để thầy/cô hiểu được cách em suy nghĩ
                      nhé!
                    </span>
                  </div>
                      ...
                      <div className="bg-white rounded-lg p-6 max-w-sm w-full">
                        ...
                        <div className="space-y-3">
                          <input
                            ...
                            onChange={(e) =>
                              setBai3BaiLamFractionNum(e.target.value)
                            }
                            placeholder="Tử số"
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setBai3BaiLamFractionDen(e.target.value)
                            }
                            placeholder="Mẫu số"
                            className="w-full px-4 py-2 border-2 border-green-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                          />
                          {(bai3BaiLamFractionNum || bai3BaiLamFractionDen) && (
                            <div className="p-3 bg-green-50 rounded-lg border-2 border-green-200">
                              ...
                              <p className="text-lg font-bold text-green-700">
                                ({bai3BaiLamFractionNum || "?"})/(
                                {bai3BaiLamFractionDen || "?"})
                              </p>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="bg-white rounded-lg p-6 max-w-sm w-full">
                        ...
                        <div className="space-y-3">
                          <input
                            ...
                            onChange={(e) =>
                              setBai3GiaiThichFractionNum(e.target.value)
                            }
                            placeholder="Tử số"
                            className="w-full px-4 py-2 border-2 border-green-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                          />
                          <input
                            ...
                            onChange={(e) =>
                              setBai3GiaiThichFractionDen(e.target.value)
                            }
                            placeholder="Mẫu số"
                            className="w-full px-4 py-2 border-2 border-green-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                          />
                          {(bai3GiaiThichFractionNum ||
                            bai3GiaiThichFractionDen) && (
                            <div className="p-3 bg-green-50 rounded-lg border-2 border-green-200">
                              <p className="text-sm text-gray-600 mb-1">
                                Preview:
                              </p>
              ...
              {worksheetData.bai_4 && (
                <div className="bg-gradient-to-br from-orange-50 to-yellow-100 rounded-3xl shadow-lg p-8 mb-8 border-4 border-orange-300 relative">
                  ...
                  <div className="space-y-8">
                    {(worksheetData.bai_4.questions || []).map(
                      (question, idx) => (
                        <div
                          key={question.id}
                        ...
                        >
                          <h3 className="text-2xl font-bold text-orange-700 mb-4 flex items-center gap-2">
                            <span className="bg-orange-300 text-white w-8 h-8 rounded-full flex items-center justify-center text-lg font-bold">
                              {idx + 1}
                            </span>
                                  ...
                                  >
                                    <div className="flex items-center justify-between gap-2 mb-3">
                                      <label className="block font-bold text-orange-700 flex items-center gap-2 flex-1">
                                        <span className="bg-orange-200 text-orange-700 px-2 py-1 rounded-full text-sm font-bold">
                                          Câu {subIdx + 1}
                                        </span>
                                    ...
                                    <textarea
                                      value={
                                        bai4Answers[question.id]?.[subIdx] || ""
                                      }
                                      onChange={(e) => {
                                        const newAnswers = { ...bai4Answers };
                                      ...
                                      disabled={isAlreadySubmitted}
                                      placeholder="✍️ Nhập đáp án của em... (Shift+Enter để xuống hàng)"
                                      ...
                                      rows="3"
                                      name={`bai4_${question.id}_subquestion_${subIdx}`}
                                    />
                                  </div>
                                ),
                              )}
                          ...
                          {question.type === "so_cach_giai" && (
                            <div className="space-y-4">
                              {/* eslint-disable-next-line no-unused-vars */}
                              {Array.from({
                                length: parseInt(question.content),
                              }).map((_, cacheIdx) => (
                                <div
                                  key={cacheIdx}
                                  className="p-4 bg-gradient-to-r from-orange-50 to-yellow-50 rounded-xl border-2 border-orange-200 hover:border-orange-400 transition"
                                >
                                  <div className="flex items-center justify-between gap-2 mb-3">
                                    <label className="block font-bold text-orange-700 flex items-center gap-2">
                                      <span className="bg-orange-200 text-orange-700 px-3 py-1 rounded-full text-sm font-bold">
                                        🌟 Cách {cacheIdx + 1}
                                      </span>
                                    </label>
                                    <button
                                  ...
                                  <textarea
                                    ...
                                    onKeyDown={handleTextareaKeyDown}
                                    disabled={isAlreadySubmitted}
                                    placeholder="✍️ Nhập đáp án cho cách này... (Shift+Enter để xuống hàng)"
                                    className="w-full px-4 py-3 border-2 border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent resize-none font-medium disabled:bg-gray-100"
                                    rows="4"
                                    name={`bai4_${question.id}_solution_${cacheIdx}`}
                                  />
                                </div>
                              ))}
                            </div>
                          ...
                          {!question.type && (
                            <div className="p-4 bg-gradient-to-r from-orange-50 to-yellow-50 rounded-xl border-2 border-orange-200 hover:border-orange-400 transition">
                              ...
                              <textarea
                                ...
                                className="w-full px-4 py-3 border-2 border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent resize-none font-medium disabled:bg-gray-100"
                                rows="3"
                                name={`bai4_${question.id}_default_0`}
                              />
                            </div>
                          )}
                        </div>
